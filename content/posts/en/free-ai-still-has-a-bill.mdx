...
date: "2026-09-23"
description: "Anthropic and OpenAI just pushed frontier-class models down the price curve on the same day. The important number is still not the token price. It is the cost of a completed workflow after context, retries, tools, latency, and human review."
...
## Update: the price war reached the frontier

The meter moved again.

This time, two labs moved it on the same day.

On September 22, Anthropic [introduced Claude Opus 5.5](https://www.anthropic.com/claude-opus-5-5), its first model release since calling for the frontier to be paced. Anthropic says it performs around the level of Claude Fable 5.1 on most work, generates output more than 30% faster than Opus 5, and costs about 40% less on typical workloads.

OpenAI released GPT-6 Sol and GPT-6 Luna hours later. Its [model catalog](https://developers.openai.com/api/docs/models) positions Sol for complex coding and agentic workflows and Luna for focused, high-volume work. Its [API pricing page](https://developers.openai.com/api/docs/pricing) lists standard short-context rates of $2 per million input tokens and $10 per million output tokens for Sol, and $0.10 and $0.50 for Luna.

That is not a normal model launch day.

It is the model market telling us what the competition is becoming.

The frontier still matters.

But capability is sliding down the price curve fast enough that the more important question is no longer just "which model is smartest?"

It is:

How much useful work survives the invoice?

## Forty percent cheaper is not a token price

Anthropic's numbers make the distinction unusually clear.

The posted token rates for Opus 5.5 are 20% below Opus 5: input falls from $5 to $4 per million tokens, while output falls from $25 to $20. Anthropic reaches its larger 40% typical-workload claim by adding another factor: Opus 5.5 uses fewer tokens to complete the work.

That may be true.

It is also a vendor estimate based on Anthropic's tests, default settings, and idea of a typical workload. Your codebase, tools, prompts, retry behavior, review gates, and failure modes did not participate in that average.

The useful part is not the exact marketing percentage.

It is the accounting model.

A stronger model can cost less even when its nominal token price is higher, if it finishes in fewer steps, avoids retries, calls fewer tools, writes less unnecessary output, and creates less rework for a human.

A cheap model can be expensive if it confidently takes the scenic route.

This is why cost per token is becoming the least interesting honest number on the pricing page.

## Cache pricing is product strategy

The most revealing Opus 5.5 price cut is not input or output.

It is cache reads.

Anthropic cut them from $0.50 to $0.20 per million tokens, a 60% reduction.

That matters because agent work is repetitive in a very specific way. A coding agent rereads repository instructions, architecture notes, tool definitions, files, test output, and conversation history. A research agent carries source material and a growing working set. An office agent keeps the same policies, templates, customer context, and approval rules across many steps.

The user sees one task.

The agent keeps bringing the same backpack through the turnstile.

Cheap cache reads make long sessions, persistent context, and repeated tool use more economical. They also reward products that structure reusable context cleanly instead of resending an improvised pile of text on every call.

This is not only infrastructure optimization.

It changes which agent products can have a margin.

## The premium model is becoming a workflow step

OpenAI's new spread makes the routing logic impossible to ignore.

GPT-6 Sol and Luna both support tools, reasoning controls, a context window above one million tokens, and large outputs. But Luna's listed input price is one-twentieth of Sol's, while Sol is positioned for the harder agentic work.

That does not mean Luna is the right answer for everything.

It means "use the best model" is no longer a serious architecture.

A real workflow might use a cheap model to classify requests, extract fields, monitor changes, and prepare context. It might call a stronger model for planning, ambiguous judgment, or a difficult code change. It might escalate one consequential decision to a frontier model and then require human approval before an external action.

The premium model becomes a step in the workflow, not the workflow itself.

That is how AI gets cheaper in production without every task becoming worse.

It is also how pricing becomes product design.

## Pacing is not a commercial ceasefire

Anthropic emphasizes that Opus 5.5 was tested by external evaluators before release and performed better on its internal behavioral audit. That is relevant after the company's call to pace frontier development.

But "pacing" clearly does not mean freezing the product line.

Anthropic and OpenAI released competing, cheaper agent models within hours of each other. The commercial race is still running. The practical version of pacing will therefore be judged by release gates, independent access, deployment safeguards, monitoring, and what happens when evaluations find a problem.

Not by the absence of launches.

That is a separate governance argument.

For buyers, the immediate lesson is simpler:

Do not confuse a lower price with a stable cost.

## Measure the workflow before celebrating the discount

These releases are genuinely good news for builders.

More capable models at lower prices make more automation viable. They give teams room to run better evaluations, try more routes, preserve more context, and automate work that did not survive the economics six months ago.

They also make it easier to hide bad economics behind cheap tokens.

The practical checklist is now:

- measure cost per successful workflow, not cost per API call
- separate fresh input, cached context, output, tool fees, and fast-mode premiums
- record retries, clarification loops, and failed runs
- include human review and correction time
- test several effort levels instead of defaulting to maximum reasoning
- route routine steps to cheaper models only after quality evals
- pin model versions and rerun evals before migration
- model the bill when usage grows by 10x

If a vendor says a model is 40% cheaper, useful.

Now run your task.

The pricing page is an input.

The completed workflow is the receipt.
