...
date: "2026-08-26"
description: "Anthropic showed the primitives production agents need. Google is now packaging the same idea by industry with Gemini Enterprise for Legal and Financial Services. The useful signal is that agents are becoming governed workflow software: skills, connectors, permissions, citations, audit trails, and domain-specific control planes."
...
## Update: the platform is becoming industry software

Google just made the same point from the other direction.

On August 25, Google Cloud launched [Gemini Enterprise for Legal](https://cloud.google.com/blog/products/ai-machine-learning/introducing-gemini-enterprise-for-legal) and [Gemini Enterprise for Financial Services](https://cloud.google.com/blog/products/ai-machine-learning/introducing-gemini-enterprise-for-financial-services), both in preview.

The lazy headline is:

"Google made AI for lawyers and bankers."

True. Too small.

The better headline is:

Agent platforms are turning into industry software.

Google's legal package is not described as a general chatbot with a legal prompt. It is a bundle of purpose-built skills, MCP connectors, partner agents, and a governed control plane. It connects to systems like iManage, NetDocuments, Docusign, Everlaw, RelativityOne, Thomson Reuters HighQ, Harvey, Legora, and CourtListener. Google says the connectors inherit existing role-based access controls, document permissions, and ethical walls, and that outputs are grounded with traceable citations.

That is the production-agent thesis, dressed for a profession that can actually sue you.

The financial-services version says the same thing in another vocabulary: domain skills, licensed data, permissioned data, explainability, confidence scores, methodologies, data snapshots for auditing, A2A APIs, MCP connectors, and reports in the formats teams already use.

Again: not a demo.

A control system.

The customer names and workflow claims should be treated as vendor claims. Preview products are not proof of real-world ROI. Business Insider's reporting also makes the competitive context clear: Google is moving into a legal AI market where Anthropic, OpenAI, Microsoft, Thomson Reuters, Harvey, Legora, and the incumbent document systems are already fighting for position.

But the product shape matters.

The agent is no longer the center of the page.

The workflow is.

For buyers, that changes the diligence:

Do not only ask which model is inside.

Ask which systems it can touch, which permissions travel with the data, which citations can be audited, which human approvals remain, which partner agent did what, and whether the output is grounded in your source of truth or just fluently adjacent to it.

For builders, the message is less comfortable:

Generic agents are getting squeezed from both sides. The model labs will offer broad platforms. Incumbent systems will expose their own data and actions through connectors. The valuable middle is not "we have an agent." It is the domain package: the playbooks, integrations, review paths, pricing discipline, and liability-aware workflow that make the agent boring enough to buy.

Very glamorous. The future of agents is procurement vocabulary.
