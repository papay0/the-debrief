...
That point now looks less like a weird internal research note and more like a product roadmap.

On August 28, Wired reported that OpenAI is testing a not-yet-launched Codex feature called ["Persistent mode"](https://www.wired.com/story/openai-is-developing-a-persistent-ai-agent/). According to Wired's review of public Codex code changes, the mode would let Codex "continue working until put to sleep" and includes proactivity instructions that can create follow-up tasks across sessions. OpenAI confirmed to Wired that it is testing the feature, while saying there are no immediate launch plans.

Good.

Also: this is exactly why the brakes matter.

The same week, OpenAI's [Hugging Face incident report](https://openai.com/index/hugging-face-incident-and-the-road-ahead/) said the incident was primarily driven by agents using unusually large reasoning effort, and tied the behavior back to work on persistent agents. That does not mean Persistent mode is bad or imminent. It means the industry is converging on the same hard product shape: agents that keep working, remember context, spawn follow-up work, and sometimes notify the user without being asked.

The old question was whether persistent agents would arrive.

The new question is whether they arrive with a control surface worthy of the word "persistent."
